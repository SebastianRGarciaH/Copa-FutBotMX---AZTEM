...
def limpiar_ruido_lateral(img):
    """Borra el texto amarillo del video original y sus manchas en las máscaras."""
    if img is not None:
        cv2.rectangle(img, (0, 0), (1200, 250), (0, 0, 0), -1)
    return img
...
def generar_video_demo():
    ...
    vid_angle_path = os.path.join(root_dir, 'data', 'partido_angle.mp4')
    ...
    lista_angle_masks = [limpiar_ruido_lateral(cv2.imread(os.path.join(root_dir, 'results', 'angle_view', f'frame_{i:05d}_angle.png'))) for i in range(1, 5)]

    grid_masks = crear_grid_4_imagenes(lista_masks, ANCHO, ALTO)
    grid_angle_masks = crear_grid_4_imagenes(lista_angle_masks, ANCHO, ALTO)

    cap_orig = cv2.VideoCapture(vid_path)
    cap_angle = cv2.VideoCapture(vid_angle_path)
    ...
    # --- FASE 2: Vista Aérea (20s) [0:05 - 0:25] ---
    print("Renderizando Fase 2 (20s)...")
    ...
    # --- FASE 3: Vista Lateral (20s) [0:25 - 0:45] ---
    print("Renderizando Fase 3 (20s)...")
    cap_angle.set(cv2.CAP_PROP_POS_FRAMES, 0)
    for _ in range(20 * FPS):
        f_angle = limpiar_ruido_lateral(read_frame_safe(cap_angle))
        frame_split = crear_split_screen(f_angle, grid_angle_masks, ANCHO, ALTO)
        
        frame_split = poner_etiqueta_esquina(frame_split, "Vista Lateral - Original", 'izq')
        frame_split = poner_etiqueta_esquina(frame_split, "Segmentacion 2x2", 'der')
        frame_split = dibujar_transicion_minimalista(frame_split)
        frame_split = poner_subtitulo(frame_split, "Procesamiento simultaneo de multiples perspectivas.")
        out.write(frame_split)

    # --- FASE 4: Tracking (30s) [0:45 - 1:15] ---
    print("Renderizando Fase 4 (30s)...")
    cap_orig.set(cv2.CAP_PROP_POS_FRAMES, 0) 
    ...
    # --- FASE 5: Visualizaciones (40s) [1:15 - 1:55] ---
    print("Renderizando Fase 5 (40s)...")
    if not visualizaciones_cargadas:
        ...
        for _ in range(40 * FPS): out.write(frame_err)
    else:
        frames_por_imagen = (40 * FPS) // len(visualizaciones_cargadas)
        ...
        frames_restantes = (40 * FPS) - (frames_por_imagen * len(visualizaciones_cargadas))
        if frames_restantes > 0 and visualizaciones_cargadas:
            for _ in range(frames_restantes): out.write(frame_vis)

    # --- FASE 6: Cierre (5s) [1:55 - 2:00] ---
    print("Renderizando Fase 6 (5s)...")
